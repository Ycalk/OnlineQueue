...
class OnRequestArchived(BaseEventHandler[RequestArchived]):
    ...
    async def __call__(self, event: RequestArchived) -> None:
        self.logger.info(f"Request {event.request_id} archived with status {event.status}")
        ...
        try:
            ...
            message = status_messages.get(event.status, f"📦 Заявка архивирована (статус: {event.status})")
            message += f"\n\nID: {event.request_id}"
